...
var merkletree = require('@thirdweb-dev/merkletree');
...
async function parseSnapshotInputs(inputs) {
  ...
  const results = [];
  const parsedChunks = await Promise.all(chunks.map(chunk => index.SnapshotInputSchema.parseAsync(chunk)));
...
class ShardedMerkleTree {
  ...
  static hashEntry(entry, tokenDecimals, currencyDecimals, snapshotFormatVersion) {
    switch (snapshotFormatVersion) {
      ...
      case SnapshotFormatVersion.V2:
        return ethers.utils.solidityKeccak256(["address", "uint256", "uint256", "address"], [entry.address, convertQuantityToBigNumber(entry.maxClaimable, tokenDecimals), convertQuantityToBigNumber(entry.price || "unlimited", currencyDecimals), entry.currencyAddress || ethers.constants.AddressZero]);
    }
  }
  static async fetchAndCacheDecimals(cache, provider, currencyAddress) {
    ...
    if (currencyDecimals === undefined) {
      const currencyMetadata = await fetchCurrencyValue.fetchCurrencyMetadata(provider, currencyAddress);
  ...
  static async buildAndUpload(snapshotInput, tokenDecimals, provider, storage, snapshotFormatVersion) {
    ...
    const subTrees = await Promise.all(Object.entries(shards).map(async _ref => {
      ...
      return [shard, new merkletree.MerkleTree(await Promise.all(entries.map(async entry => {
        // cache decimals for each currency to avoid refetching for every address
    ...
    const tree = new merkletree.MerkleTree(Object.values(roots), ethers.utils.keccak256, {
      sort: true
    });
  ...
  async getProof(address, provider, snapshotFormatVersion) {
    ...
    if (shard === undefined) {
      try {
        ...
        this.trees[shardId] = new merkletree.MerkleTree(hashedEntries, ethers.utils.keccak256, {
          sort: true
        });
      } catch (e) {
        return null;
    ...
    return index.SnapshotEntryWithProofSchema.parseAsync({
      ...entry,
...
async function fetchSnapshotEntryForAddress(address, merkleRoot, merkleMetadata, provider, storage, snapshotFormatVersion) {
  ...
  if (snapshotUri) {
    ...
    const snapshotData = await index.SnapshotSchema.parseAsync(raw);
...
async function approveErc20Allowance(contractToApprove, currencyAddress, price, quantity, tokenDecimals) {
  ...
  const totalPrice = ethers.BigNumber.from(price).mul(ethers.BigNumber.from(quantity)).div(ethers.utils.parseUnits("1", tokenDecimals));
...
async function prepareClaim(addressToClaim, quantity, activeClaimCondition, merkleMetadataFetcher, tokenDecimals, contractWrapper, storage, checkERC20Allowance, snapshotFormatVersion) {
  ...
  try {
    if (!activeClaimCondition.merkleRootHash.toString().startsWith(ethers.constants.AddressZero)) {
      ...
      if (snapshotEntry) {
        ...
        priceInProof = snapshotEntry.price === undefined || snapshotEntry.price === "unlimited" ? ethers.constants.MaxUint256 : await normalizePriceValue.normalizePriceValue(contractWrapper.getProvider(), snapshotEntry.price, snapshotEntry.currencyAddress || ethers.constants.AddressZero);
  ...
  if (pricePerToken.gt(0)) {
    if (fetchCurrencyValue.isNativeToken(currencyAddress)) {
      overrides["value"] = ethers.BigNumber.from(pricePerToken).mul(quantity).div(ethers.utils.parseUnits("1", tokenDecimals));
 ...
 * @internal
 */
const CurrencySchema = /* @__PURE__ */(() => zod.z.object({
  name: zod.z.string(),
  symbol: zod.z.string(),
 ...
 * @internal
 */
const ClaimConditionMetadataSchema = /* @__PURE__ */(() => zod.z.object({
 ...
 * @internal
 */
const ClaimConditionInputSchema = /* @__PURE__ */(() => zod.z.object({
  startTime: assertEnabled.StartDateSchema,
  currencyAddress: zod.z.string().default(index.NATIVE_TOKEN_ADDRESS),
  price: index.AmountSchema.default(0),
  maxClaimableSupply: index.QuantitySchema,
  maxClaimablePerWallet: index.QuantitySchema,
  waitInSeconds: index.BigNumberishSchema.default(0),
  merkleRootHash: index.BytesLikeSchema.default(ethers.utils.hexZeroPad([0], 32)),
  snapshot: zod.z.optional(index.SnapshotInputSchema).nullable(),
 ...
 * @internal
 */
const ClaimConditionInputArray = /* @__PURE__ */(() => zod.z.array(ClaimConditionInputSchema))();
...
const ClaimConditionOutputSchema = /* @__PURE__ */(() => ClaimConditionInputSchema.extend({
  availableSupply: index.QuantitySchema,
  currentMintSupply: index.QuantitySchema,
  ...
  price: index.BigNumberSchema,
  waitInSeconds: index.BigNumberSchema,
  startTime: index.BigNumberSchema.transform(n => new Date(n.toNumber() * 1000)),
  snapshot: index.SnapshotInputSchema.optional().nullable()
}))();
...
async function createSnapshot(snapshotInput, tokenDecimals, provider, storage, snapshotFormatVersion) {
  ...
  if (hasDuplicates) {
    throw new index.DuplicateLeafsError();
  }
...
async function createMerkleTreeFromAllowList(snapshotInput) {
  ...
  const tree = new merkletree.MerkleTree(leaves, ethers.utils.keccak256, {
    sort: true
  });
  return tree;
}
...
async function convertToContractModel(c, tokenDecimals, provider, storage) {
  const currency = c.currencyAddress === ethers.constants.AddressZero ? index.NATIVE_TOKEN_ADDRESS : c.currencyAddress;
  ...
  return {
    ...
    pricePerToken: await normalizePriceValue.normalizePriceValue(provider, c.price, currency),
    currency,
...
async function transformResultToClaimCondition(pm, tokenDecimals, provider, merkleMetadata, storage, shouldDownloadSnapshot) {
  const cv = await fetchCurrencyValue.fetchCurrencyValue(provider, pm.currency, pm.pricePerToken);
 ...
 * @internal
 */
const BaseSignaturePayloadInput = /* @__PURE__ */(() => zod.z.object({
  to: index.AddressOrEnsSchema.refine(address => address.toLowerCase() !== ethers.constants.AddressZero, {
    message: "Cannot create payload to mint to zero address"
  }),
  price: index.AmountSchema.default(0),
  currencyAddress: index.AddressSchema.default(index.NATIVE_TOKEN_ADDRESS),
  mintStartTime: assertEnabled.StartDateSchema,
  mintEndTime: assertEnabled.EndDateSchema,
  uid: zod.z.string().optional().transform(arg => resolveOrGenerateId(arg)),
  primarySaleRecipient: index.AddressOrEnsSchema.default(ethers.constants.AddressZero)
}))();
...
const Signature20PayloadOutput = /* @__PURE__ */(() => Signature20PayloadInput.extend({
  mintStartTime: index.BigNumberSchema,
  mintEndTime: index.BigNumberSchema
}))();
...
const Signature721PayloadOutput = /* @__PURE__ */(() => Signature721PayloadInput.extend({
  metadata: setErc20Allowance.NFTInputOrUriSchema.default(""),
  uri: zod.z.string(),
  royaltyBps: index.BigNumberSchema,
  mintStartTime: index.BigNumberSchema,
  mintEndTime: index.BigNumberSchema
}))();
...
const Signature1155PayloadInput = /* @__PURE__ */(() => Signature721PayloadInput.extend({
  metadata: setErc20Allowance.NFTInputOrUriSchema.default(""),
  quantity: index.BigNumberishSchema
}))();
...
const Signature1155PayloadOutput = /* @__PURE__ */(() => Signature721PayloadOutput.extend({
  tokenId: index.BigNumberSchema,
  quantity: index.BigNumberSchema
}))();
...
exports.SnapshotFormatVersion = SnapshotFormatVersion;
exports.abstractContractModelToLegacy = abstractContractModelToLegacy;
exports.abstractContractModelToNew = abstractContractModelToNew;
exports.approveErc20Allowance = approveErc20Allowance;
exports.convertQuantityToBigNumber = convertQuantityToBigNumber;
exports.convertToReadableQuantity = convertToReadableQuantity;
exports.createMerkleTreeFromAllowList = createMerkleTreeFromAllowList;
exports.createSnapshot = createSnapshot;
exports.fetchSnapshotEntryForAddress = fetchSnapshotEntryForAddress;
exports.getProofsForAllowListEntry = getProofsForAllowListEntry;
exports.hashAllowListEntry = hashAllowListEntry;
exports.legacyContractModelToAbstract = legacyContractModelToAbstract;
exports.newContractModelToAbstract = newContractModelToAbstract;
exports.prepareClaim = prepareClaim;
exports.processClaimConditionInputs = processClaimConditionInputs;
exports.resolveOrGenerateId = resolveOrGenerateId;
exports.transformResultToClaimCondition = transformResultToClaimCondition;
exports.updateExistingClaimConditions = updateExistingClaimConditions;
