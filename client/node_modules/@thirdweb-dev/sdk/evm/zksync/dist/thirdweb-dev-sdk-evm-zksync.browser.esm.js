...
import { aJ as fetchPublishedContractFromPolygon, aK as fetchAndCacheDeployMetadata, a1 as extractConstructorParamsFromAbi, dC as getRoyaltyEngineV1ByChainId, aI as THIRDWEB_DEPLOYER, a$ as getChainProvider, Y as fetchContractMetadata, ao as fetchSourceFilesFromMetadata } from '../../../dist/index-454de556.browser.esm.js';
import { f as checkVerificationStatus, T as ThirdwebSDK, e as extractFunctionParamsFromAbi, t as convertParamValues } from '../../../dist/sdk-f6ebef2b.browser.esm.js';
import { constants, utils, Contract as Contract$1 } from 'ethers';
import invariant from 'tiny-invariant';
import { parseEther } from 'ethers/lib/utils';
import { hashBytecode, create2Address } from 'zksync-ethers/build/utils';
import 'zod';
...
import '../../../dist/contract-publisher-325971b7.browser.esm.js';
import '@thirdweb-dev/contracts-js/dist/abis/ContractPublisher.json';
import '../../../dist/transactions-eae32dda.browser.esm.js';
import '@thirdweb-dev/contracts-js/dist/abis/TWMultichainRegistryLogic.json';
import '@thirdweb-dev/contracts-js/dist/abis/TWMultichainRegistryRouter.json';
import '../../../dist/fetchCurrencyValue-f87e5d3a.browser.esm.js';
import '../../../dist/normalizePriceValue-8ce073fb.browser.esm.js';
import '@thirdweb-dev/generated-abis';
import '../../../dist/deployContractDeterministic-672a3924.browser.esm.js';
...
async function zkDeployCreate2Factory(signer) {
  ...
  const create2FactoryDeploy = new ContractFactory(zkSingletonFactoryArtifact.abi, zkSingletonFactoryArtifact.bytecode, create2Signer, "create");
...
function zkComputeDeploymentAddress(bytecode, encodedArgs, create2FactoryAddress, salt) {
  ...
  const addr = create2Address(create2FactoryAddress, bytecodeHash, saltHash, encodedArgs);
  return addr;
}
...
async function zkComputeDeploymentInfo(contractType, provider, storage, create2Factory, contractOptions, clientId, secretKey) {
  ...
  if (!metadata) {
    ...
    metadata = (await fetchAndCacheDeployMetadata(publishedContract.metadataUri, storage, {
      compilerType: "zksolc"
    })).compilerMetadata;
  }
...
async function zkVerify(contractAddress, chainId, explorerAPIUrl, explorerAPIKey, storage, metadataUri, zkVersion, encodedConstructorArgs) {
  try {
    ...
    if (data.status === RequestStatus.OK) {
      ...
      const verificationStatus = await checkVerificationStatus(explorerAPIUrl, explorerAPIKey, data.result);
...
async function zkFetchConstructorParams(explorerAPIUrl, explorerAPIKey, contractAddress, abi, provider) {
  ...
  if (creationTx && creationTx.status === RequestStatus.OK && creationTx.result[0] !== undefined) {
    ...
    if (transaction.to === "0x0000000000000000000000000000000000008006") {
      const decoded = utils.defaultAbiCoder.decode(["bytes32", "bytes32", "bytes"], utils.hexDataSlice(transaction.data, 4));
...
async function fetchFromMultiChainRegistry(address, chainId) {
  try {
    ...
    const uri = await sdk.multiChainRegistry.getContractMetadataURI(chainId, address);
    return uri;
  } catch (e) {}
}
...
async function zkDeployContractDeterministic(signer, transaction, storage, metadataUri, options) {
  ...
  if (!contractDeployed) {
    ...
    if (parsedMarker !== "1") {
      try {
        const implFactory = new ContractFactory(transaction.abi, transaction.bytecode, signer, "create");
    ...
    const singleton = new Contract(transaction.to || "0xa51baf6a9c0ef5Db8C1898d5aDD92Bf3227d6088", singletonDeployAbi, signer);
    options?.notifier?.("deploying", "preset");
    try {
      const deployTx = await singleton.deploy(utils.id("thirdweb"), transaction.bytecodeHash, transaction.constructorCalldata);
      await deployTx.wait();
...
async function registerContractOnMultiChainRegistry(address, chainId, metadataURI) {
  try {
    ...
    const existingMeta = await sdk.multiChainRegistry.getContractMetadataURI(chainId, address);
...
async function zkDeployContractFromUri(publishMetadataUri, constructorParamValues, signer, storage, chainId, options, clientId, secretKey) {
  const {
    compilerMetadata,
    extendedMetadata
  } = await fetchAndCacheDeployMetadata(publishMetadataUri, storage, options?.compilerOptions);
  ...
  if (extendedMetadata && extendedMetadata.factoryDeploymentData && !forceDirectDeploy && (!extendedMetadata.deployType || extendedMetadata.deployType !== "standard")) {
    if (extendedMetadata.isDeployableViaProxy || extendedMetadata.isDeployableViaFactory || extendedMetadata.deployType === "autoFactory") {
      ...
      const encodedInitializer = Contract$1.getInterface(compilerMetadata.abi).encodeFunctionData(extendedMetadata.factoryDeploymentData.implementationInitializerFunction, paramValues);
      const proxyFactory = new ContractFactory(twProxyArtifactZK.abi, twProxyArtifactZK.bytecode, signer, "create");
    ...
    const factory = new ContractFactory(compilerMetadata.abi, compilerMetadata.bytecode, signer, "create");
...
export { CONTRACT_ADDRESSES, IMPLEMENTATIONS, NATIVE_TOKENS, NATIVE_TOKEN_ADDRESS, blockExplorerApiMap, getImplementation, getZkNativeTokenByChainId, getZkTransactionsForDeploy, zkDeployContractFromUri, zkGetDefaultTrustedForwarders, zkVerify };
