...
async function getUserOpReceipt(chain, userOpHash) {
  ...
  const readOnlySDK = getSDK(chain);
  const entrypoint = await readOnlySDK.getContract(entryPointAddress || constants.ENTRYPOINT_ADDRESS, contracts.EntryPoint__factory.abi);
...
class SmartWallet extends evm_wallets_base_dist_thirdwebDevWalletsEvmWalletsBase.AbstractClientWallet {
  ...
  async verifySignature(message, signature, address, chainId) {
    ...
    if (chainId !== undefined) {
      try {
        return await constants.checkContractWalletSignature(message, signature, address, chainId || 1, this.options?.clientId, this.options?.secretKey);
      } catch {
        // no-op
      }
    }
