...
var url = require('../../../../dist/url-8b7f19db.cjs.prod.js');
var headers = require('../../../../dist/headers-b16c2eb6.cjs.prod.js');
var constants = require('../../../../dist/constants-50e5d28e.cjs.prod.js');
var contracts = require('@account-abstraction/contracts');
var defineProperty = require('../../../../dist/defineProperty-b749763b.cjs.prod.js');
...
class HttpRpcClient {
  ...
  async estimateUserOpGas(userOp) {
    ...
    return {
      ...
      callGasLimit: ethers.BigNumber.from(data.callGasLimit).add(constants.MANAGED_ACCOUNT_GAS_BUFFER)
    };
  }
...
Code: ${errorCode}`;
  ...
  async signMessage(message) {
    ...
    const isValid = await constants.checkContractWalletSignature(message, signature, address, chainId, this.config.clientId, this.config.secretKey);
    if (isValid) {
      return signature;
    } else {
...
class SmartWalletConnector extends connector.Connector {
  ...
  async getAccountContract() {
    ...
    if (this.config.accountInfo?.abi) {
      ...
      return sdk$1.getContract(await this.getAddress(), constants.ACCOUNT_CORE_ABI);
    }
  }
  ...
  async estimateTx(tx, options) {
    ...
    const [userOp, gasPrice] = await Promise.all([this.accountApi.createUnsignedUserOp(this.aaProvider.httpRpcClient, tx, options), sdk.getGasPrice(provider)]);
    const resolved = await ethers.utils.resolveProperties(userOp);
    const transactionGasLimit = ethers.BigNumber.from(resolved.callGasLimit);
  ...
  async prepareBatchRaw(transactions) {
    ...
    const targets = resolvedTxs.map(tx => tx.to || ethers.constants.AddressZero);
