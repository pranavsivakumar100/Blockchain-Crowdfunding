...
class LocalWallet extends evm_wallets_base_dist_thirdwebDevWalletsEvmWalletsBase.AbstractClientWallet {
  ...
  constructor(options) {
    ...
    this._storage = options?.storage || evm_wallets_base_dist_thirdwebDevWalletsEvmWalletsBase.createAsyncLocalStorage(walletIds.walletIds.localWallet);
  }
  async getConnector() {
    if (!this.connector) {
      const {
...
function getDecryptor(encryption) {
  ...
  (encryption.decrypt || crypto.aesDecryptCompat)(msg, encryption.password) : noop;
}
...
function getEncryptor(encryption) {
  return encryption ? msg => (encryption.encrypt || crypto.aesEncrypt)(msg, encryption.password) : noop;
}
