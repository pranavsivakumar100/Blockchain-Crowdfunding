...
var marketplace = require('./marketplace-54e28e3e.cjs.prod.js');
...
var contractPlatformFee = require('./contract-platform-fee-b8249aaa.cjs.prod.js');
var contractRoles = require('./contract-roles-61e7c2a3.cjs.prod.js');
var cleanCurrencyAddress = require('./cleanCurrencyAddress-76ee5675.cjs.prod.js');
...
var ListingType = require('./ListingType-ac81d896.cjs.prod.js');
require('zod');
...
class MarketplaceAuction {
  constructor(contractWrapper, storage) {
    ...
    this.encoder = new fetchCurrencyValue.ContractEncoder(contractWrapper);
  }
  getAddress() {
  ...
  async getListing(listingId) {
    ...
    if (listing.listingId.toString() !== listingId.toString()) {
      throw new index.ListingNotFoundError(this.getAddress(), listingId.toString());
    }
    if (listing.listingType !== ListingType.ListingType.Auction) {
      throw new index.WrongListingTypeError(this.getAddress(), listingId.toString(), "Direct", "Auction");
    }
  ...
  async getWinningBid(listingId) {
    ...
    return await marketplace.mapOffer(this.contractWrapper.getProvider(), ethers.BigNumber.from(listingId), offers);
  }
  ...
  async getWinner(listingId) {
    ...
    const contractEvents = new contractAppuri.ContractEvents(this.contractWrapper);
  ...
  createListing = /* @__PURE__ */transactions.buildTransactionFunction(async listing => {
    marketplace.validateNewListingParam(listing);
    const resolvedAssetAddress = await index.resolveAddress(listing.assetContractAddress);
    const resolvedCurrencyAddress = await index.resolveAddress(listing.currencyContractAddress);
    await marketplace.handleTokenApproval(this.contractWrapper, this.getAddress(), resolvedAssetAddress, listing.tokenId, await this.contractWrapper.getSignerAddress());
    const normalizedPricePerToken = await normalizePriceValue.normalizePriceValue(this.contractWrapper.getProvider(), listing.buyoutPricePerToken, resolvedCurrencyAddress);
    const normalizedReservePrice = await normalizePriceValue.normalizePriceValue(this.contractWrapper.getProvider(), listing.reservePricePerToken, resolvedCurrencyAddress);
    ...
    const tx = transactions.Transaction.fromContractWrapper({
      ...
      args: [{
        ...
        currencyToAccept: cleanCurrencyAddress.cleanCurrencyAddress(resolvedCurrencyAddress),
   ...
   * ```
   */
  buyoutListing = /* @__PURE__ */transactions.buildTransactionFunction(async listingId => {
    const listing = await this.validateListing(ethers.BigNumber.from(listingId));
    const currencyMetadata = await fetchCurrencyValue.fetchCurrencyMetadata(this.contractWrapper.getProvider(), listing.currencyContractAddress);
   ...
   * ```
   */
  makeBid = /* @__PURE__ */transactions.buildTransactionFunction(async (listingId, pricePerToken) => {
    const listing = await this.validateListing(ethers.BigNumber.from(listingId));
    const normalizedPrice = await normalizePriceValue.normalizePriceValue(this.contractWrapper.getProvider(), pricePerToken, listing.currencyContractAddress);
    ...
    if (winningBid) {
      const isWinner = marketplace.isWinningBid(winningBid.pricePerToken, normalizedPrice, bidBuffer);
  ...
  cancelListing = /* @__PURE__ */transactions.buildTransactionFunction(async listingId => {
    ...
    if (now.gt(startTime) && offers.offeror !== ethers.constants.AddressZero) {
      throw new index.AuctionAlreadyStartedError(listingId.toString());
    }
    const tx = transactions.Transaction.fromContractWrapper({
      contractWrapper: this.contractWrapper,
      method: "closeAuction",
   ...
   */
  closeListing = /* @__PURE__ */transactions.buildTransactionFunction(async (listingId, closeFor) => {
    if (!closeFor) {
    ...
    try {
      ...
      if (err.message.includes("cannot close auction before it has ended")) {
        throw new index.AuctionHasNotEndedError(listingId.toString(), listing.endTimeInEpochSeconds.toString());
      } else {
        throw err;
      }
    }
   ...
   * @param listingId - the auction  listing ud to close
   */
  executeSale = /* @__PURE__ */transactions.buildTransactionFunction(async listingId => {
    const listing = await this.validateListing(ethers.BigNumber.from(listingId));
    try {
      const winningBid = await this.getWinningBid(listingId);
   ...
   */
  updateListing = /* @__PURE__ */transactions.buildTransactionFunction(async listing => {
    const tx = transactions.Transaction.fromContractWrapper({
      contractWrapper: this.contractWrapper,
  ...
  async getMinimumNextBid(listingId) {
    ...
    const [currentBidBufferBps, winningBid, listing] = await Promise.all([this.getBidBufferBps(), this.getWinningBid(listingId), this.validateListing(ethers.BigNumber.from(listingId))]);
    ...
    return fetchCurrencyValue.fetchCurrencyValue(this.contractWrapper.getProvider(), listing.currencyContractAddress, minimumNextBid);
  }
  ...
  async mapListing(listing) {
    return {
      ...
      buyoutCurrencyValuePerToken: await fetchCurrencyValue.fetchCurrencyValue(this.contractWrapper.getProvider(), listing.currency, listing.buyoutPricePerToken),
      ...
      reservePriceCurrencyValuePerToken: await fetchCurrencyValue.fetchCurrencyValue(this.contractWrapper.getProvider(), listing.currency, listing.reservePricePerToken),
      ...
      type: ListingType.ListingType.Auction
    };
  }
}
...
class MarketplaceDirect {
  ...
  async getListing(listingId) {
    ...
    if (listing.listingType !== ListingType.ListingType.Direct) {
      throw new index.WrongListingTypeError(this.getAddress(), listingId.toString(), "Auction", "Direct");
    }
    return await this.mapListing(listing);
  }
  ...
  async getActiveOffer(listingId, address) {
    ...
    const offers = await this.contractWrapper.read("offers", [listingId, await index.resolveAddress(address)]);
    if (offers.offeror === ethers.constants.AddressZero) {
      return undefined;
    }
    return await marketplace.mapOffer(this.contractWrapper.getProvider(), ethers.BigNumber.from(listingId), offers);
  ...
  createListing = /* @__PURE__ */transactions.buildTransactionFunction(async listing => {
    ...
    const tx = transactions.Transaction.fromContractWrapper({
      ...
      args: [{
        ...
        buyoutPricePerToken: normalizedPricePerToken,
        currencyToAccept: cleanCurrencyAddress.cleanCurrencyAddress(resolvedCurrencyAddress),
        listingType: ListingType.ListingType.Direct,
        quantityToList: listing.quantity,
   ...
   * ```
   */
  makeOffer = /* @__PURE__ */transactions.buildTransactionFunction(async (listingId, quantityDesired, currencyContractAddress, pricePerToken, expirationDate) => {
    if (fetchCurrencyValue.isNativeToken(currencyContractAddress)) {
      throw new Error("You must use the wrapped native token address when making an offer with a native token");
    }
    const normalizedPrice = await normalizePriceValue.normalizePriceValue(this.contractWrapper.getProvider(), pricePerToken, currencyContractAddress);
    try {
    ...
    const overrides = (await this.contractWrapper.getCallOverrides()) || {};
    await setErc20Allowance.setErc20Allowance(this.contractWrapper, value, currencyContractAddress, overrides);
   ...
   * ```
   */
  acceptOffer = /* @__PURE__ */transactions.buildTransactionFunction(async (listingId, addressOfOfferor) => {
    /**
     * TODO:
     * - Provide better error handling if offer is too low.
     */
    await this.validateListing(ethers.BigNumber.from(listingId));
    const resolvedAddress = await index.resolveAddress(addressOfOfferor);
   ...
   */
  buyoutListing = /* @__PURE__ */transactions.buildTransactionFunction(async (listingId, quantityDesired, receiver) => {
    const listing = await this.validateListing(ethers.BigNumber.from(listingId));
    const {
      valid,
      error
  ...
  updateListing = /* @__PURE__ */transactions.buildTransactionFunction(async listing => {
    const tx = transactions.Transaction.fromContractWrapper({
      ...
      listing.buyoutPrice, await index.resolveAddress(listing.currencyContractAddress), listing.startTimeInSeconds, listing.secondsUntilEnd]
    });
    tx.setGasLimitMultiple(1.2);
    return tx;
  });
  ...
  async mapListing(listing) {
    return {
      ...
      sellerAddress: listing.tokenOwner,
      type: ListingType.ListingType.Direct
    };
  }
  ...
  async isStillValidListing(listing, quantity) {
    const approved = await marketplace.isTokenApprovedForTransfer(this.contractWrapper.getProvider(), this.getAddress(), listing.assetContractAddress, listing.tokenId, listing.sellerAddress);
    if (!approved) {
      return {
        valid: false,
...
class Marketplace {
  ...
  constructor(network, address, storage) {
    ...
    this.roles = new contractRoles.ContractRoles(this.contractWrapper, Marketplace.contractRoles);
  ...
  async getListing(listingId) {
    ...
    switch (listing.listingType) {
      case ListingType.ListingType.Auction:
        {
          return await this.auction.mapListing(listing);
        }
      case ListingType.ListingType.Direct:
        {
  ...
  async getActiveListings(filter) {
    ...
    return filtered.filter(l => {
      return l.type === ListingType.ListingType.Auction && ethers.BigNumber.from(l.endTimeInEpochSeconds).gt(now) && ethers.BigNumber.from(l.startTimeInEpochSeconds).lte(now) || l.type === ListingType.ListingType.Direct && ethers.BigNumber.from(l.quantity).gt(0);
    });
  }
  ...
  async isRestrictedToListerRoleOnly() {
    const anyoneCanList = await this.contractWrapper.read("hasRole", [index.getRoleHash("lister"), ethers.constants.AddressZero]);
  ...
  async getOffers(listingId) {
    ...
    return await Promise.all(listingEvents.map(e => {
      return marketplace.mapOffer(this.contractWrapper.getProvider(), ethers.BigNumber.from(listingId), {
   ...
   * ```
   */
  makeOffer = /* @__PURE__ */transactions.buildTransactionFunction(async (listingId, pricePerToken, quantity) => {
    const listing = await this.contractWrapper.read("listings", [listingId]);
    if (listing.listingId.toString() !== listingId.toString()) {
      throw new index.ListingNotFoundError(this.getAddress(), listingId.toString());
    }
      ...
      case ListingType.ListingType.Direct:
        {
          ...
          return await this.direct.makeOffer.prepare(listingId, quantity, fetchCurrencyValue.isNativeToken(listing.currency) ? index.NATIVE_TOKENS[chainId].wrapped.address : listing.currency, pricePerToken);
        }
      case ListingType.ListingType.Auction:
        {
   ...
   */
  setBidBufferBps = /* @__PURE__ */transactions.buildTransactionFunction(async bufferBps => {
   ...
   */
  setTimeBufferInSeconds = /* @__PURE__ */transactions.buildTransactionFunction(async bufferInSeconds => {
    await this.roles.verify(["admin"], await this.contractWrapper.getSignerAddress());
   ...
   */
  allowListingFromSpecificAssetOnly = /* @__PURE__ */transactions.buildTransactionFunction(async contractAddress => {
    const encoded = [];
    const members = await this.roles.get("asset");
    if (members.includes(ethers.constants.AddressZero)) {
      encoded.push(this.encoder.encode("revokeRole", [index.getRoleHash("asset"), ethers.constants.AddressZero]));
    }
    encoded.push(this.encoder.encode("grantRole", [index.getRoleHash("asset"), contractAddress]));
    return transactions.Transaction.fromContractWrapper({
      contractWrapper: this.contractWrapper,
      method: "multicall",
      args: [encoded]
   ...
   */
  allowListingFromAnyAsset = /* @__PURE__ */transactions.buildTransactionFunction(async () => {
    const encoded = [];
    const members = await this.roles.get("asset");
    for (const addr in members) {
      encoded.push(this.encoder.encode("revokeRole", [index.getRoleHash("asset"), addr]));
    }
    encoded.push(this.encoder.encode("grantRole", [index.getRoleHash("asset"), ethers.constants.AddressZero]));
    return transactions.Transaction.fromContractWrapper({
      contractWrapper: this.contractWrapper,
      method: "multicall",
      args: [encoded]
  ...
  async getAllListingsNoFilter(filterInvalidListings) {
    const listings = await Promise.all(Array.from(Array((await this.contractWrapper.read("totalListings", [])).toNumber()).keys()).map(async i => {
      ...
      try {
        ...
        if (err instanceof index.ListingNotFoundError) {
          return undefined;
        } else {
      ...
      if (listing.type === ListingType.ListingType.Auction) {
        return listing;
      }
