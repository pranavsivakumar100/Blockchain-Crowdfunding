...
var hasERC20Allowance = require('./hasERC20Allowance-78c900f2.cjs.prod.js');
...
var erc1155Standard = require('./erc-1155-standard-9c981132.cjs.prod.js');
var IPackVRFAbi = require('@thirdweb-dev/contracts-js/dist/abis/IPackVRFDirect.json');
var erc20 = require('./erc-20-dac4d97b.cjs.prod.js');
var pack = require('./pack-ad6c4c26.cjs.prod.js');
require('zod');
...
require('./setErc20Allowance-49111a78.cjs.prod.js');
require('./signature-d916937d.cjs.prod.js');
require('@thirdweb-dev/merkletree');
require('uuid');
require('./erc-1155-08d3a5b3.cjs.prod.js');
require('fast-deep-equal');
require('./drop-claim-conditions-95bbb50b.cjs.prod.js');
...
class PackVRF {
  featureName = index.FEATURE_PACK_VRF.name;
  constructor(network, address, storage, options, chainId) {
    let contractWrapper = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : new index.ContractWrapper(network, address, IPackVRFAbi__default["default"], options, storage);
   ...
   * @twfeature PackVRF
   */
  open = /* @__PURE__ */transactions.buildTransactionFunction((() => {
    var _this = this;
   ...
   * @twfeature PackVRF
   */
  claimRewards = /* @__PURE__ */transactions.buildTransactionFunction((() => {
    var _this2 = this;
  ...
  async parseRewards(rewards) {
    ...
    for (const reward of rewards) {
      switch (reward.tokenType) {
        case 0:
          {
            const tokenMetadata = await fetchCurrencyValue.fetchCurrencyMetadata(this.contractWrapper.getProvider(), reward.assetContract);
            erc20Rewards.push({
  ...
  async canClaimRewards(claimerAddress) {
    const address = await index.resolveAddress(claimerAddress || (await this.contractWrapper.getSignerAddress()));
  ...
  getLinkContract(ERC20Abi) {
    const linkAddress = index.LINK_TOKEN_ADDRESS[this.chainId];
    if (!linkAddress) {
      throw new Error(`No LINK token address found for chainId ${this.chainId}`);
    }
    const contract = new index.ContractWrapper(this.contractWrapper.getSignerOrProvider(), linkAddress, ERC20Abi, this.contractWrapper.options, this.storage);
    return new erc20.Erc20(contract, this.storage, this.chainId);
  }
}
...
class Pack extends erc1155Standard.StandardErc1155 {
  static contractRoles = index.PACK_CONTRACT_ROLES;
   ...
   */
  get vrf() {
    return assertEnabled.assertEnabled(this._vrf, index.FEATURE_PACK_VRF);
  }
  constructor(network, address, storage) {
    let options = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : {};
    let abi = arguments.length > 4 ? arguments[4] : undefined;
    let chainId = arguments.length > 5 ? arguments[5] : undefined;
    let contractWrapper = arguments.length > 6 && arguments[6] !== undefined ? arguments[6] : new index.ContractWrapper(network, address, abi, options.gasless && "openzeppelin" in options.gasless ? {
      ...options,
      gasless: {
        ...options.gasless,
        openzeppelin: {
    ...
    this.metadata = new contractAppuri.ContractMetadata(this.contractWrapper, index.PackContractSchema, this.storage);
    this.app = new contractAppuri.ContractAppURI(this.contractWrapper, this.metadata, this.storage);
    this.roles = new contractRoles.ContractRoles(this.contractWrapper, Pack.contractRoles);
        ...
        case 0:
          {
            ...
            const totalRewards = ethers.utils.formatUnits(ethers.BigNumber.from(reward.totalAmount).div(amount), tokenMetadata.decimals);
            erc20Rewards.push({
              contractAddress: reward.assetContract,
              quantityPerReward,
              totalRewards
   ...
   * ```
   */
  create = /* @__PURE__ */transactions.buildTransactionFunction(async metadataWithRewards => {
   ...
   * ```
   */
  addPackContents = /* @__PURE__ */transactions.buildTransactionFunction(async (packId, packContents) => {
    const [signerAddress, parsedContents] = await Promise.all([this.contractWrapper.getSignerAddress(), pack.PackRewardsOutputSchema.parseAsync(packContents)]);
    const {
      contents,
      numOfRewardUnits
   ...
   * ```
   */
  createTo = /* @__PURE__ */transactions.buildTransactionFunction(async (to, metadataWithRewards) => {
    const [uri, parsedMetadata, toAddress] = await Promise.all([QueryParams.uploadOrExtractURI(metadataWithRewards.packMetadata, this.storage), pack.PackMetadataInputSchema.parseAsync(metadataWithRewards), index.resolveAddress(to)]);
    const {
      erc20Rewards,
      erc721Rewards,
      erc1155Rewards
        ...
        parse: async receipt => {
          ...
          for (const reward of rewards) {
            switch (reward.tokenType) {
              case 0:
                {
                  const tokenMetadata = await fetchCurrencyValue.fetchCurrencyMetadata(_this.contractWrapper.getProvider(), reward.assetContract);
                  erc20Rewards.push({
                    contractAddress: reward.assetContract,
                    quantityPerReward: ethers.utils.formatUnits(reward.totalAmount, tokenMetadata.decimals).toString()
                  });
  ...
  async toPackContentArgs(metadataWithRewards) {
    ...
    for (const erc20 of erc20Rewards) {
      const normalizedQuantity = await normalizePriceValue.normalizePriceValue(provider, erc20.quantityPerReward, erc20.contractAddress);
      ...
      const hasAllowance = await hasERC20Allowance.hasERC20Allowance(this.contractWrapper, erc20.contractAddress, totalQuantity);
      if (!hasAllowance) {
    ...
    for (const erc721 of erc721Rewards) {
      const isApproved = await marketplace.isTokenApprovedForTransfer(this.contractWrapper.getProvider(), this.getAddress(), erc721.contractAddress, erc721.tokenId, owner);
      if (!isApproved) {
    ...
    for (const erc1155 of erc1155Rewards) {
      const isApproved = await marketplace.isTokenApprovedForTransfer(this.contractWrapper.getProvider(), this.getAddress(), erc1155.contractAddress, erc1155.tokenId, owner);
      if (!isApproved) {
  ...
  detectVrf() {
    if (contractAppuri.detectContractFeature(this.contractWrapper, "PackVRF")) {
