...
function detectContractFeature(contractWrapper, featureName) {
  const b = index.isExtensionEnabled(contractWrapper.abi, featureName, contractWrapper.extensions);
  return b;
}
...
class ContractMetadata {
  featureName = index.FEATURE_METADATA.name;
  ...
  async get() {
    ...
    if (!data) {
      try {
        ...
        try {
          publishedMetadata = await index.fetchContractMetadataFromAddress(this.contractWrapper.address, this.contractWrapper.getProvider(), this.storage, this.contractWrapper.options);
        } catch (err) {}
        data = {
  ...
  set = /* @__PURE__ */transactions.buildTransactionFunction(async metadata => {
    ...
    if (this.supportsContractMetadata(wrapper)) {
      ...
      throw new index.ExtensionNotImplementedError(index.FEATURE_METADATA);
    }
  });
   ...
   * @twfeature ContractMetadata
   * */
  update = /* @__PURE__ */transactions.buildTransactionFunction(async metadata => {
...
class ContractEvents {
  ...
  addTransactionListener(listener) {
    this.contractWrapper.addListener(index.EventType.Transaction, listener);
  }
  ...
  removeTransactionListener(listener) {
    this.contractWrapper.off(index.EventType.Transaction, listener);
  }
...
class GasCostEstimator {
  ...
  // eslint-disable-next-line @typescript-eslint/ban-types
  fn, args) {
    const gasCost = await index.estimateTransactionCost(this.contractWrapper.getProvider(), await this.contractWrapper.populateTransaction(fn, args));
...
class ContractAppURI {
  featureName = index.FEATURE_APPURI.name;
  ...
  async get() {
    ...
    return storage.replaceGatewayUrlWithScheme((await this.metadata.get()).app_uri || "", this.storage.getGatewayUrls());
  }
   ...
   * @twfeature AppURI
   */
  set = /* @__PURE__ */transactions.buildTransactionFunction(async appURI => {
    if (detectContractFeature(this.contractWrapper, "AppURI")) {
      return transactions.Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
        method: "setAppURI",
