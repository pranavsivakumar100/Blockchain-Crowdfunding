...
var index = require('./index-486ba778.cjs.prod.js');
var assertEnabled = require('./assertEnabled-1fc1968d.cjs.prod.js');
var contractAppuri = require('./contract-appuri-68c23622.cjs.prod.js');
var QueryParams = require('./QueryParams-9f8e0457.cjs.prod.js');
var transactions = require('./transactions-9107d452.cjs.prod.js');
var zod = require('zod');
var contractOwner = require('./contract-owner-d440b992.cjs.prod.js');
var deepEqual = require('fast-deep-equal');
var signature = require('./signature-d916937d.cjs.prod.js');
var fetchCurrencyValue = require('./fetchCurrencyValue-5d19eddf.cjs.prod.js');
var invariant = require('tiny-invariant');
var normalizePriceValue = require('./normalizePriceValue-45f2b228.cjs.prod.js');
var setErc20Allowance = require('./setErc20Allowance-49111a78.cjs.prod.js');
...
class DropErc1155ClaimConditions {
  ...
  async getActive(tokenId, options) {
    ...
    return await signature.transformResultToClaimCondition(mc, 0, this.contractWrapper.getProvider(), metadata.merkle, this.storage, options?.withAllowList || false);
  }
  ...
  async getAll(tokenId, options) {
    if (this.isLegacyMultiPhaseDrop(this.contractWrapper) || this.isNewMultiphaseDrop(this.contractWrapper)) {
      ...
      const metadata = await this.metadata.get();
      return Promise.all(conditions.map(c => signature.transformResultToClaimCondition(c, 0, this.contractWrapper.getProvider(), metadata.merkle, this.storage, options?.withAllowList || false)));
    } else {
  ...
  async getClaimIneligibilityReasons(tokenId, quantity, addressToCheck) {
    ...
    const resolvedAddress = await index.resolveAddress(addressToCheck);
    try {
    ...
    if (this.isNewSinglePhaseDrop(this.contractWrapper) || this.isNewMultiphaseDrop(this.contractWrapper)) {
      ...
      let maxClaimable = signature.convertQuantityToBigNumber(claimCondition.maxClaimablePerWallet, 0);
      try {
      ...
      if (allowListEntry) {
        maxClaimable = signature.convertQuantityToBigNumber(allowListEntry.maxClaimable, 0);
      }
    ...
    if (claimCondition.price.gt(0) && index.isNode()) {
      ...
      if (fetchCurrencyValue.isNativeToken(claimCondition.currencyAddress)) {
        ...
        const IERC20ABI = (await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require('@thirdweb-dev/contracts-js/dist/abis/IERC20.json')); })).default;
        const erc20 = new index.ContractWrapper(provider, claimCondition.currencyAddress, IERC20ABI, {}, this.storage);
  ...
  async getClaimerProofs(tokenId, claimerAddress, claimConditionId) {
    ...
    if (merkleRootArray.length > 0) {
      const metadata = await this.metadata.get();
      const resolvedAddress = await index.resolveAddress(claimerAddress);
   ...
   * @param resetClaimEligibilityForAll - Whether to reset the state of who already claimed NFTs previously
   */
  setBatch = /* @__PURE__ */transactions.buildTransactionFunction((() => {
    var _this2 = this;
    return async function (claimConditionsForToken) {
      ...
      const processedClaimConditions = await Promise.all(claimConditionsForToken.map(async _ref => {
        ...
        const {
          snapshotInfos,
          sortedConditions
        } = await signature.processClaimConditionInputs(claimConditionsProcessed, 0, _this2.contractWrapper.getProvider(), _this2.storage, _this2.getSnapshotFormatVersion());
      ...
      if (!deepEqual__default["default"](metadata.merkle, merkleInfo)) {
        ...
        if (contractAppuri.hasFunction("setContractURI", _this2.contractWrapper)) {
          const contractEncoder = new fetchCurrencyValue.ContractEncoder(_this2.contractWrapper);
      ...
      processedClaimConditions.forEach(_ref2 => {
        ...
        const baseContractEncoder = new fetchCurrencyValue.ContractEncoder(_this2.contractWrapper);
        if (_this2.isLegacySinglePhaseDrop(_this2.contractWrapper)) {
          const legacyContractEncoder = new fetchCurrencyValue.ContractEncoder(_this2.contractWrapper);
          encoded.push(legacyContractEncoder.encode("setClaimConditions", [tokenId, signature.abstractContractModelToLegacy(sortedConditions[0]), resetClaimEligibilityForAll]));
        } else if (_this2.isLegacyMultiPhaseDrop(_this2.contractWrapper)) {
          encoded.push(baseContractEncoder.encode("setClaimConditions", [tokenId, sortedConditions.map(signature.abstractContractModelToLegacy), resetClaimEligibilityForAll]));
        } else if (_this2.isNewSinglePhaseDrop(_this2.contractWrapper)) {
          encoded.push(baseContractEncoder.encode("setClaimConditions", [tokenId, signature.abstractContractModelToNew(sortedConditions[0]), resetClaimEligibilityForAll]));
        } else if (_this2.isNewMultiphaseDrop(_this2.contractWrapper)) {
          encoded.push(baseContractEncoder.encode("setClaimConditions", [tokenId, sortedConditions.map(signature.abstractContractModelToNew), resetClaimEligibilityForAll]));
        } else {
          throw new Error("Contract does not support claim conditions");
        }
      });
      if (contractAppuri.hasFunction("multicall", _this2.contractWrapper)) {
   ...
   */
  update = /* @__PURE__ */transactions.buildTransactionFunction(async (tokenId, index, claimConditionInput) => {
  ...
  async prepareClaim(tokenId, quantity, checkERC20Allowance, address) {
    const addressToClaim = await index.resolveAddress(address ? address : await this.contractWrapper.getSignerAddress());
    return signature.prepareClaim(addressToClaim, quantity, await this.getActive(tokenId), async () => (await this.metadata.get()).merkle, 0, this.contractWrapper, this.storage, checkERC20Allowance, this.getSnapshotFormatVersion());
  }
  ...
  isNewSinglePhaseDrop(contractWrapper) {
    return contractAppuri.detectContractFeature(contractWrapper, "ERC1155ClaimConditionsV2");
  }
  isNewMultiphaseDrop(contractWrapper) {
    return contractAppuri.detectContractFeature(contractWrapper, "ERC1155ClaimPhasesV2");
  }
  isLegacySinglePhaseDrop(contractWrapper) {
    return contractAppuri.detectContractFeature(contractWrapper, "ERC1155ClaimConditionsV1");
  }
  isLegacyMultiPhaseDrop(contractWrapper) {
    return contractAppuri.detectContractFeature(contractWrapper, "ERC1155ClaimPhasesV1");
  }
 ...
 * @internal
 */
const AirdropAddressInput = /* @__PURE__ */(() => zod.z.object({
  address: index.AddressOrEnsSchema,
  quantity: index.AmountSchema.default(1)
}))();

/**
 * @internal
 */
const AirdropInputSchema = /* @__PURE__ */(() => zod.z.union([zod.z.array(zod.z.string()).transform(async strings => await Promise.all(strings.map(address => AirdropAddressInput.parseAsync({
  address
...
class ERC1155Claimable {
  featureName = index.FEATURE_EDITION_CLAIM_CUSTOM.name;
  ...
  async getClaimTransaction(destinationAddress, tokenId, quantity, options) {
    ...
    if (options && options.pricePerToken) {
      overrides = await contractOwner.calculateClaimCost(this.contractWrapper, options.pricePerToken, quantity, options.currencyAddress, options.checkERC20Allowance);
    }
    return transactions.Transaction.fromContractWrapper({
      contractWrapper: this.contractWrapper,
      method: "claim",
      args: [await index.resolveAddress(destinationAddress), tokenId, quantity],
      overrides
    });
  }
   ...
   */
  to = /* @__PURE__ */transactions.buildTransactionFunction(async (destinationAddress, tokenId, quantity, options) => {
...
class Erc1155ClaimableWithConditions {
  featureName = index.FEATURE_EDITION_CLAIM_CONDITIONS_V2.name;
...
class Erc1155SignatureMintable {
  featureName = index.FEATURE_EDITION_SIGNATURE_MINTABLE.name;
   ...
   */
  mint = /* @__PURE__ */transactions.buildTransactionFunction(async signedPayload => {
    ...
    await setErc20Allowance.setErc20Allowance(this.contractWrapper, message.pricePerToken.mul(message.quantity), mintRequest.currencyAddress, overrides);
    return transactions.Transaction.fromContractWrapper({
      contractWrapper: this.contractWrapper,
   ...
   * @twfeature ERC1155SignatureMintable
   */
  mintBatch = /* @__PURE__ */transactions.buildTransactionFunction(async signedPayloads => {
    ...
    if (contractAppuri.hasFunction("multicall", this.contractWrapper)) {
      return transactions.Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
        method: "multicall",
        args: [encoded],
  ...
  async generateBatchFromTokenIds(payloadsToSign) {
    ...
    const parsedRequests = await Promise.all(payloadsToSign.map(m => signature.Signature1155PayloadInputWithTokenId.parseAsync(m)));
    const metadatas = parsedRequests.map(r => r.metadata);
    const [uris, chainId, contractInfo] = await Promise.all([QueryParams.uploadOrExtractURIs(metadatas, this.storage), this.contractWrapper.getChainID(), index.getPrebuiltInfo(this.contractWrapper.address, this.contractWrapper.getProvider())]);
    const finalPayloads = await Promise.all(parsedRequests.map((m, i) => signature.Signature1155PayloadOutput.parseAsync({
      ...m,
      uri: uris[i]
    })));
    ...
    const signatures = await Promise.all(contractStructs.map(contractStruct => this.contractWrapper.signTypedData(signer, {
      ...
      MintRequest: signature.MintRequest1155
    },
    // TYPEHASH
    contractStruct)));
  ...
  async mapPayloadToContractStruct(mintRequest) {
    const normalizedPricePerToken = await normalizePriceValue.normalizePriceValue(this.contractWrapper.getProvider(), mintRequest.price, mintRequest.currencyAddress);
    return {
      to: mintRequest.to,
...
class Erc1155Burnable {
  featureName = index.FEATURE_EDITION_BURNABLE.name;
  constructor(contractWrapper) {
    this.contractWrapper = contractWrapper;
  }
   ...
   * ```
   */
  tokens = /* @__PURE__ */transactions.buildTransactionFunction(async (tokenId, amount) => {
   ...
   * ```
   */
  from = /* @__PURE__ */transactions.buildTransactionFunction(async (account, tokenId, amount) => {
    return transactions.Transaction.fromContractWrapper({
      contractWrapper: this.contractWrapper,
      method: "burn",
      args: [await index.resolveAddress(account), tokenId, amount]
    });
  });
   ...
   * ```
   */
  batch = /* @__PURE__ */transactions.buildTransactionFunction(async (tokenIds, amounts) => {
    const account = await this.contractWrapper.getSignerAddress();
   ...
   * ```
   */
  batchFrom = /* @__PURE__ */transactions.buildTransactionFunction(async (account, tokenIds, amounts) => {
    return transactions.Transaction.fromContractWrapper({
      contractWrapper: this.contractWrapper,
      method: "burnBatch",
      args: [await index.resolveAddress(account), tokenIds, amounts]
    });
  });
}
...
class Erc1155Enumerable {
  featureName = index.FEATURE_EDITION_ENUMERABLE.name;
  ...
  async owned(walletAddress, queryParams) {
    const [address, maxId] = await Promise.all([index.resolveAddress(walletAddress || (await this.contractWrapper.getSignerAddress())), this.contractWrapper.read("nextTokenIdToMint", [])]);
    ...
    if (queryParams) {
      ...
      const count = queryParams?.count || QueryParams.DEFAULT_QUERY_ALL_COUNT;
...
class Erc1155LazyMintable {
  featureName = index.FEATURE_EDITION_LAZY_MINTABLE_V2.name;
   ...
   */
  lazyMint = /* @__PURE__ */transactions.buildTransactionFunction(async (metadatas, options) => {
    const startFileNumber = await this.erc1155.nextTokenIdToMint();
    const batch = await QueryParams.uploadOrExtractURIs(metadatas, this.storage, startFileNumber.toNumber(), options);
    ...
    const prebuiltInfo = await index.getPrebuiltInfo(this.contractWrapper.address, this.contractWrapper.getProvider());
  ...
  updateMetadata = /* @__PURE__ */transactions.buildTransactionFunction(async (tokenId, metadata, options) => {
    ...
    let startTokenId = ethers.BigNumber.from(0);
    let endTokenId = ethers.BigNumber.from(0);
    let batchIndex = 0;
    ...
    for (let i = 0; i < metadatas.length; i++) {
      ...
      if (ethers.BigNumber.from(targetTokenId).eq(ethers.BigNumber.from(id))) {
    ...
    const batch = await QueryParams.uploadOrExtractURIs(newMetadatas, this.storage, startTokenId.toNumber(), options);
    const baseUri = batch[0].substring(0, batch[0].lastIndexOf("/"));
  ...
  detectErc1155Revealable() {
    if (contractAppuri.detectContractFeature(this.contractWrapper, "ERC1155Revealable")) {
      return new contractOwner.DelayedReveal(this.contractWrapper, this.storage, index.FEATURE_EDITION_REVEALABLE.name, () => this.erc1155.nextTokenIdToMint());
    }
    return undefined;
  }
...
class Erc1155BatchMintable {
  featureName = index.FEATURE_EDITION_BATCH_MINTABLE.name;
  constructor(erc1155, contractWrapper, storage) {
    this.erc1155 = erc1155;
    this.contractWrapper = contractWrapper;
    this.storage = storage;
   ...
   * ```
   */
  to = /* @__PURE__ */transactions.buildTransactionFunction(async (to, metadataWithSupply) => {
    const metadatas = metadataWithSupply.map(a => a.metadata);
    const supplies = metadataWithSupply.map(a => a.supply);
    const uris = await QueryParams.uploadOrExtractURIs(metadatas, this.storage);
    const resolvedAddress = await index.resolveAddress(to);
    const contractEncoder = new fetchCurrencyValue.ContractEncoder(this.contractWrapper);
    const encoded = await Promise.all(uris.map(async (uri, index) => contractEncoder.encode("mintTo", [resolvedAddress, ethers.constants.MaxUint256, uri, supplies[index]])));
    return transactions.Transaction.fromContractWrapper({
      contractWrapper: this.contractWrapper,
      method: "multicall",
      args: [encoded],
...
class Erc1155Mintable {
  featureName = index.FEATURE_EDITION_MINTABLE.name;
  ...
  async getMintTransaction(to, metadataWithSupply) {
    const uri = await QueryParams.uploadOrExtractURI(metadataWithSupply.metadata, this.storage);
    return transactions.Transaction.fromContractWrapper({
      contractWrapper: this.contractWrapper,
      method: "mintTo",
      args: [await index.resolveAddress(to), ethers.constants.MaxUint256, uri, metadataWithSupply.supply]
    });
  }
   ...
   * ```
   */
  additionalSupplyTo = /* @__PURE__ */transactions.buildTransactionFunction(async (to, tokenId, additionalSupply) => {
    const metadata = await this.erc1155.getTokenMetadata(tokenId);
    return transactions.Transaction.fromContractWrapper({
      contractWrapper: this.contractWrapper,
      method: "mintTo",
      args: [await index.resolveAddress(to), tokenId, metadata.uri, additionalSupply],
      parse: receipt => {
        return {
  ...
  detectErc1155BatchMintable() {
    if (contractAppuri.detectContractFeature(this.contractWrapper, "ERC1155BatchMintable")) {
 ...
 */
class Erc1155 {
  featureName = index.FEATURE_EDITION.name;
  get chainId() {
   ...
   */
  async get(tokenId) {
    const [supply, metadata] = await Promise.all([this.contractWrapper.read("totalSupply", [tokenId]).catch(() => ethers.BigNumber.from(0)), this.getTokenMetadata(tokenId).catch(() => ({
      id: tokenId.toString(),
      uri: "",
      ...QueryParams.FALLBACK_METADATA
    }))]);
    return {
  ...
  async totalSupply(tokenId) {
    if (contractAppuri.detectContractFeature(this.contractWrapper, "ERC1155Supply")) {
      return await this.contractWrapper.read("totalSupply", [tokenId]);
    } else {
      throw new index.ExtensionNotImplementedError(index.FEATURE_EDITION_SUPPLY);
    }
  }
  ...
  async balanceOf(address, tokenId) {
    return await this.contractWrapper.read("balanceOf", [await index.resolveAddress(address), tokenId]);
  }
  ...
  async isApproved(address, operator) {
    return await this.contractWrapper.read("isApprovedForAll", [await index.resolveAddress(address), await index.resolveAddress(operator)]);
  }
   ...
   * @twfeature ERC1155
   */
  transfer = /* @__PURE__ */transactions.buildTransactionFunction((() => {
    var _this = this;
    return async function (to, tokenId, amount) {
      ...
      return transactions.Transaction.fromContractWrapper({
        ...
        args: [from, await index.resolveAddress(to), tokenId, amount, data]
      });
    };
  })());
   ...
   */
  transferBatch = /* @__PURE__ */transactions.buildTransactionFunction((() => {
    var _this2 = this;
    return async function (to, tokenIds, amounts, fromAddress) {
      let data = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : [0];
      const from = fromAddress ? await index.resolveAddress(fromAddress) : await _this2.contractWrapper.getSignerAddress();
      return transactions.Transaction.fromContractWrapper({
        contractWrapper: _this2.contractWrapper,
        method: "safeBatchTransferFrom",
        args: [from, await index.resolveAddress(to), tokenIds, amounts, data]
      });
    };
  })());
   ...
   * @twfeature ERC1155
   */
  transferFrom = /* @__PURE__ */transactions.buildTransactionFunction((() => {
    var _this3 = this;
    return async function (from, to, tokenId, amount) {
      ...
      return transactions.Transaction.fromContractWrapper({
        ...
        method: "safeTransferFrom",
        args: [await index.resolveAddress(from), await index.resolveAddress(to), tokenId, amount, data]
      });
    };
  })());
   ...
   * @twfeature ERC1155BatchTransferable
   */
  airdrop = /* @__PURE__ */transactions.buildTransactionFunction((() => {
    var _this4 = this;
    return async function (tokenId, addresses, fromAddress) {
      let data = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : [0];
      const from = fromAddress ? await index.resolveAddress(fromAddress) : await _this4.contractWrapper.getSignerAddress();
      ...
      const totalToAirdrop = input.reduce((prev, curr) => {
        return ethers.BigNumber.from(prev).add(ethers.BigNumber.from(curr?.quantity || 1));
      }, ethers.BigNumber.from(0));
      if (balanceOf.lt(ethers.BigNumber.from(totalToAirdrop))) {
        throw new Error(`The caller owns ${balanceOf.toString()} NFTs, but wants to airdrop ${totalToAirdrop.toString()} NFTs.`);
      }
      const contractEncoder = new fetchCurrencyValue.ContractEncoder(_this4.contractWrapper);
  ...
  async nextTokenIdToMint() {
    if (contractAppuri.hasFunction("nextTokenIdToMint", this.contractWrapper)) {
      return await this.contractWrapper.read("nextTokenIdToMint", []);
    } else {
  ...
  async getAll(queryParams) {
    return assertEnabled.assertEnabled(this.query, index.FEATURE_EDITION_ENUMERABLE).all(queryParams);
  }
   ...
   */
  async totalCount() {
    return assertEnabled.assertEnabled(this.query, index.FEATURE_EDITION_ENUMERABLE).totalCount();
  }
   ...
   */
  async totalCirculatingSupply(tokenId) {
    return assertEnabled.assertEnabled(this.query, index.FEATURE_EDITION_ENUMERABLE).totalCirculatingSupply(tokenId);
  }
  ...
  async getOwned(walletAddress, queryParams) {
    ...
    return assertEnabled.assertEnabled(this.query, index.FEATURE_EDITION_ENUMERABLE).owned(walletAddress, queryParams);
  }
   ...
   */
  mint = /* @__PURE__ */transactions.buildTransactionFunction(async metadataWithSupply => {
   ...
   * @twfeature ERC1155Mintable
   */
  mintTo = /* @__PURE__ */transactions.buildTransactionFunction(async (receiver, metadataWithSupply) => {
    return assertEnabled.assertEnabled(this.mintable, index.FEATURE_EDITION_MINTABLE).to.prepare(receiver, metadataWithSupply);
  });
  ...
  async getMintTransaction(receiver, metadataWithSupply) {
    return assertEnabled.assertEnabled(this.mintable, index.FEATURE_EDITION_MINTABLE).getMintTransaction(receiver, metadataWithSupply);
  }
   ...
   * @twfeature ERC1155Mintable
   */
  mintAdditionalSupply = /* @__PURE__ */transactions.buildTransactionFunction(async (tokenId, additionalSupply) => {
    return assertEnabled.assertEnabled(this.mintable, index.FEATURE_EDITION_MINTABLE).additionalSupplyTo.prepare(await this.contractWrapper.getSignerAddress(), tokenId, additionalSupply);
  });
   ...
   * @twfeature ERC1155Mintable
   */
  mintAdditionalSupplyTo = /* @__PURE__ */transactions.buildTransactionFunction(async (receiver, tokenId, additionalSupply) => {
    return assertEnabled.assertEnabled(this.mintable, index.FEATURE_EDITION_MINTABLE).additionalSupplyTo.prepare(receiver, tokenId, additionalSupply);
  });
   ...
   */
  mintBatch = /* @__PURE__ */transactions.buildTransactionFunction(async metadataWithSupply => {
   ...
   * @twfeature ERC1155BatchMintable
   */
  mintBatchTo = /* @__PURE__ */transactions.buildTransactionFunction(async (receiver, metadataWithSupply) => {
    return assertEnabled.assertEnabled(this.mintable?.batch, index.FEATURE_EDITION_BATCH_MINTABLE).to.prepare(receiver, metadataWithSupply);
  });
   ...
   */
  burn = /* @__PURE__ */transactions.buildTransactionFunction(async (tokenId, amount) => {
    return assertEnabled.assertEnabled(this.burnable, index.FEATURE_EDITION_BURNABLE).tokens.prepare(tokenId, amount);
  });
   ...
   * @twfeature ERC1155Burnable
   */
  burnFrom = /* @__PURE__ */transactions.buildTransactionFunction(async (account, tokenId, amount) => {
    return assertEnabled.assertEnabled(this.burnable, index.FEATURE_EDITION_BURNABLE).from.prepare(account, tokenId, amount);
  });
   ...
   * @twfeature ERC1155Burnable
   */
  burnBatch = /* @__PURE__ */transactions.buildTransactionFunction(async (tokenIds, amounts) => {
    return assertEnabled.assertEnabled(this.burnable, index.FEATURE_EDITION_BURNABLE).batch.prepare(tokenIds, amounts);
  });
   ...
   * @twfeature ERC1155Burnable
   */
  burnBatchFrom = /* @__PURE__ */transactions.buildTransactionFunction(async (account, tokenIds, amounts) => {
    return assertEnabled.assertEnabled(this.burnable, index.FEATURE_EDITION_BURNABLE).batchFrom.prepare(account, tokenIds, amounts);
  });
   ...
   */
  lazyMint = /* @__PURE__ */transactions.buildTransactionFunction(async (metadatas, options) => {
    return assertEnabled.assertEnabled(this.lazyMintable, index.FEATURE_EDITION_LAZY_MINTABLE_V2).lazyMint.prepare(metadatas, options);
  });
   ...
   */
  updateMetadata = /* @__PURE__ */transactions.buildTransactionFunction(async (tokenId, metadata) => {
    if (this.lazyMintable) {
      return this.lazyMintable.updateMetadata.prepare(tokenId, metadata);
    } else if (contractAppuri.detectContractFeature(this.contractWrapper, "ERC1155UpdatableMetadata")) {
      ...
      throw new index.ExtensionNotImplementedError(index.FEATURE_EDITION_UPDATABLE_METADATA);
    }
  });
  ...
  async getClaimTransaction(destinationAddress, tokenId, quantity, options) {
    ...
    throw new index.ExtensionNotImplementedError(index.FEATURE_EDITION_CLAIM_CUSTOM);
  }
   ...
   */
  claim = /* @__PURE__ */transactions.buildTransactionFunction(async (tokenId, quantity, options) => {
   ...
   * @twfeature ERC1155ClaimCustom | ERC1155ClaimPhasesV2 | ERC1155ClaimPhasesV1 | ERC1155ClaimConditionsV2 | ERC1155ClaimConditionsV1
   */
  claimTo = /* @__PURE__ */transactions.buildTransactionFunction(async (destinationAddress, tokenId, quantity, options) => {
    const claimWithConditions = this.claimWithConditions;
    const claim = this.claimCustom;
    if (claimWithConditions) {
   ...
   */
  get claimConditions() {
    return assertEnabled.assertEnabled(this.claimWithConditions, index.FEATURE_EDITION_CLAIM_CONDITIONS_V2).conditions;
  }
   ...
   */
  get signature() {
    return assertEnabled.assertEnabled(this.signatureMintable, index.FEATURE_EDITION_SIGNATURE_MINTABLE);
  }
   ...
   */
  get revealer() {
    return assertEnabled.assertEnabled(this.lazyMintable?.revealer, index.FEATURE_EDITION_REVEALABLE);
  }
  ...
  async getTokenMetadata(tokenId) {
    ...
    return QueryParams.fetchTokenMetadata(tokenId, tokenUri, this.storage);
  }
  detectErc1155Enumerable() {
    if (contractAppuri.detectContractFeature(this.contractWrapper, "ERC1155Enumerable")) {
      return new Erc1155Enumerable(this, this.contractWrapper);
    }
  }
  detectErc1155Mintable() {
    if (contractAppuri.detectContractFeature(this.contractWrapper, "ERC1155Mintable")) {
      return new Erc1155Mintable(this, this.contractWrapper, this.storage);
    }
    return undefined;
  }
  detectErc1155Burnable() {
    if (contractAppuri.detectContractFeature(this.contractWrapper, "ERC1155Burnable")) {
      return new Erc1155Burnable(this.contractWrapper);
    }
    return undefined;
  }
  detectErc1155LazyMintable() {
    if (contractAppuri.detectContractFeature(this.contractWrapper, "ERC1155LazyMintableV1") || contractAppuri.detectContractFeature(this.contractWrapper, "ERC1155LazyMintableV2")) {
      return new Erc1155LazyMintable(this, this.contractWrapper, this.storage);
    }
    return undefined;
  }
  detectErc1155SignatureMintable() {
    if (contractAppuri.detectContractFeature(this.contractWrapper, "ERC1155SignatureMintable")) {
      return new Erc1155SignatureMintable(this.contractWrapper, this.storage);
    }
    return undefined;
  }
  detectErc1155Claimable() {
    if (contractAppuri.detectContractFeature(this.contractWrapper, "ERC1155ClaimCustom")) {
      return new ERC1155Claimable(this.contractWrapper);
    }
    return undefined;
  }
  detectErc1155ClaimableWithConditions() {
    if (contractAppuri.detectContractFeature(this.contractWrapper, "ERC1155ClaimConditionsV1") || contractAppuri.detectContractFeature(this.contractWrapper, "ERC1155ClaimConditionsV2") || contractAppuri.detectContractFeature(this.contractWrapper, "ERC1155ClaimPhasesV1") || contractAppuri.detectContractFeature(this.contractWrapper, "ERC1155ClaimPhasesV2")) {
