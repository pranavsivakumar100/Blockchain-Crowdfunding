...
import {
  IERC20_default
} from "./chunk-5GFA5ZH5.js";
import {
  IERC20Metadata_default
} from "./chunk-EGWPWJMJ.js";
import {
  IERC721_default
} from "./chunk-QWFIJIBR.js";
import {
  IERC721Metadata_default
} from "./chunk-JBLG36RJ.js";
import {
  IERC1155_default
} from "./chunk-RRKG2OFG.js";
import {
  IERC1155Metadata_default
} from "./chunk-6XWYBKHF.js";
import {
  Contract,
  ...
  "node_modules/cid-tool/src/core/bases.js"(exports, module) {
    ...
    module.exports = function bases() {
      ...
      for (const base of Object.values(multibase.names)) {
        output2.push({ name: base.name, code: base.code });
      }
      return output2;
    };
  }
...
function arrayBuffer(uint8Arr) {
  const md52 = new Md5();
  md52.update(uint8Arr);
  return md52.arrayBuffer();
}
...
function concatUint8Arrays(arrays, totalLength) {
  ...
  if (totalLength === null || totalLength === void 0) {
...
async function dangerouslyDeriveParameters(password, salt, keySizeDWORD, ivSizeDWORD, iterations) {
  ...
  const key = await crypto2.subtle.importKey("raw", rawKey, "AES-CBC", false, ["decrypt"]);
  return {
    key,
    iv
  };
...
async function aesDecrypt(ciphertext, password) {
  ...
  const pwHash = await crypto2.subtle.digest("SHA-256", pwUtf8);
  ...
  const key = await crypto2.subtle.importKey("raw", pwHash, alg, false, ["decrypt"]);
  const ctUint8 = cipherUint8Array.slice(12);
  try {
    const plainBuffer = await crypto2.subtle.decrypt(alg, key, ctUint8);
    return getCachedTextDecoder().decode(plainBuffer);
  } catch (e) {
    throw new Error("Decrypt failed");
  }
...
async function aesEncrypt(plaintext, password) {
  ...
  const key = await crypto2.subtle.importKey("raw", pwHash, alg, false, ["encrypt"]);
  const ptUint8 = textEncoder.encode(plaintext);
  const ctBuffer = await crypto2.subtle.encrypt(alg, key, ptUint8);
...
init_shim();
var import_cid_tool = __toESM(require_src7());
...
// node_modules/@thirdweb-dev/sdk/dist/transactions-eae32dda.browser.esm.js
init_shim();

// node_modules/@thirdweb-dev/sdk/dist/index-454de556.browser.esm.js
init_shim();
init_lib();
    ...
    if (match) {
      switch (match[1].toLowerCase()) {
        ...
        case "https":
          ...
          const newProvider = chainId ? new StaticJsonRpcBatchProvider({
            url: rpcUrl,
            headers,
            skipFetchSetup: _skipFetchSetup
          }, chainId, sdkOptions?.rpcBatchSettings) : new lib_exports2.JsonRpcBatchProvider({
            url: rpcUrl,
            headers,
            skipFetchSetup: _skipFetchSetup
          });
...
var CompilerTypeInput = (() => z.union([z.literal("solc"), z.literal("zksolc")]))();
...
function extractMinimalProxyImplementationAddress(bytecode2) {
  ...
  if (bytecode2.startsWith("0x365f5f375f5f365f73")) {
    const implementationAddress = bytecode2.slice(20, 60);
    return `0x${implementationAddress}`;
  }
  return void 0;
}
...
function extractFunctionsFromAbi(abi, metadata) {
  ...
  for (const f2 of functions) {
    ...
    const args = f2.inputs?.map((i) => `${i.name || "key"}: ${toJSType(i)}`)?.join(", ") || "";
    const fargs = args ? `, [${args}]` : "";
    const out = f2.outputs?.map((o) => toJSType(o, true))?.join(", ");
...
function matchesAbiFromBytecode(contractBytecode, featureAbis) {
  const interfaces = featureAbis.map((abi) => new import_utils3.Interface(abi));
...
async function fetchContractMetadataFromBytecode(address, chainId, provider2, storage) {
  ...
  const bytecode2 = ipfsData?.bytecode;
...
async function getMetadataUriFromMultichainRegistry(address, chainId, sdkOptions) {
  const TWRegistryABI = (await import("./TWMultichainRegistryLogic-G6XXN5KI.js")).default;
...
var CUSTOM_GAS_BINS = [1, 1 * 10 ** 9, 100 * 10 ** 9, 500 * 10 ** 9, 1e3 * 10 ** 9, 2500 * 10 ** 9, 5e3 * 10 ** 9, 7500 * 10 ** 9, 1e4 * 10 ** 9, 25e3 * 10 ** 9, 5e4 * 10 ** 9, 75e3 * 10 ** 9, 1e5 * 10 ** 9, 25e4 * 10 ** 9, 5e5 * 10 ** 9, 75e4 * 10 ** 9, 1e6 * 10 ** 9];
...
function getCreate2FactoryDeploymentInfo(chainId, gasOptions) {
  ...
  const gasLimit = gasOptions.gasLimit ? gasOptions.gasLimit : 1e5;
...
async function getCreate2FactoryAddress(provider2) {
  const deploymentInfo = await computeCreate2FactoryTransaction(provider2);
  return deploymentInfo.deployment;
}
async function computeCreate2FactoryTransaction(provider2) {
  const networkId = (await provider2.getNetwork()).chainId;
  if (CUSTOM_GAS_FOR_CHAIN[networkId]) {
    const enforceEip1552 = await isEIP155Enforced(provider2);
    const chainId2 = enforceEip1552 ? networkId : 0;
    const gasPrice = CUSTOM_GAS_FOR_CHAIN[networkId]?.gasPrice;
    const gasLimit = CUSTOM_GAS_FOR_CHAIN[networkId]?.gasLimit;
    const deploymentInfo = getCreate2FactoryDeploymentInfo(chainId2, {
      gasPrice: gasPrice ? BigNumber.from(gasPrice) : void 0,
      gasLimit: gasLimit ? BigNumber.from(gasLimit) : void 0
    });
    if (await isContractDeployed(deploymentInfo.deployment, provider2)) {
      deploymentInfo.transaction = "";
    }
    return deploymentInfo;
  }
  const allBinsInfo = [
    ...CUSTOM_GAS_BINS.map((b) => getCreate2FactoryDeploymentInfo(networkId, {
  ...
  const allFactories = await Promise.all(allBinsInfo.map((b) => isContractDeployed(b.deployment, provider2)));
  ...
  const [enforceEip155, gasPriceFetched] = await Promise.all([isEIP155Enforced(provider2), provider2.getGasPrice()]);
...
async function computeDeploymentInfo(contractType, provider2, storage, create2Factory, contractOptions, clientId, secretKey) {
  ...
  const encodedArgs = await encodeConstructorParamsForImplementation(metadata, provider2, storage, create2Factory, contractOptions?.constructorParams, clientId, secretKey);
...
`;
    ...
    if (info.value && info.value.gt(0)) {
      errorMessage += withSpaces("value", `${utils_exports.formatEther(info.value)} ${NATIVE_TOKENS[info.network.chainId]?.symbol || ""}`);
    }
...
var ContractWrapper = class extends RPCConnectionHandler {
  ...
  async read(functionName, args) {
    const functions = this.functions.filter((f2) => f2.name === functionName);
    if (!functions.length) {
...
Expected function signature: ${functions[0].signature}`);
  ...
  async enginePrepareRequest(transaction) {
    ...
    if (transaction.functionName === "approve" && transaction.functionArgs.length === 2) {
      ...
      const forwarderAddress = this.options.gasless.engine.relayerForwarderAddress || CONTRACT_ADDRESSES[transaction.chainId]?.openzeppelinForwarder || await computeForwarderAddress(provider2, storage);
      const ForwarderABI = (await import("./Forwarder-OZB5T7TG.js")).default;
...
var EditionDropInitializer = {
  ...
  initialize: async function() {
    ...
    const [abi, contract, _network] = await Promise.all([EditionDropInitializer.getAbi(address, provider2, storage), import("./edition-drop-297845f0.browser.esm-MPIESC4P.js"), provider2.getNetwork()]);
  ...
  getAbi: async (address, provider2, storage) => {
    ...
    return !contractInfo || contractInfo.version > 2 ? (await import("./DropERC1155-3R6WCAHM.js")).default : (await import("./DropERC1155_V2-QQPCNQVA.js")).default;
  }
};
var EditionInitializer = {
  ...
  initialize: async function() {
    ...
    const [, provider2] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([EditionInitializer.getAbi(address, provider2, storage), import("./edition-84a72cb6.browser.esm-ICEKYRQU.js"), provider2.getNetwork()]);
  ...
  getAbi: async (address, provider2, storage) => {
    ...
    return (await import("./TokenERC1155-HTAX2SOJ.js")).default;
  }
};
var MarketplaceInitializer = {
  ...
  initialize: async function() {
    ...
    const [, provider2] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([MarketplaceInitializer.getAbi(address, provider2, storage), import("./marketplace-4f435ae0.browser.esm-33SZJ3GW.js"), provider2.getNetwork()]);
  ...
  getAbi: async (address, provider2, storage) => {
    ...
    return (await import("./Marketplace-QOYPICVG.js")).default;
  }
};
var MarketplaceV3Initializer = {
  ...
  initialize: async function() {
    ...
    const [, provider2] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([MarketplaceV3Initializer.getAbi(address, provider2, storage, options), import("./marketplacev3-437b326f.browser.esm-YE6XWLVW.js"), provider2.getNetwork()]);
  ...
  getAbi: async (address, provider2, storage, options) => {
    ...
    const localAbi = (await import("./MarketplaceV3-O3YCMR5J.js")).default;
...
var MultiwrapInitializer = {
  ...
  initialize: async function() {
    ...
    const [, provider2] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([MultiwrapInitializer.getAbi(address, provider2, storage), import("./multiwrap-341d8c3d.browser.esm-HMNISIRY.js"), provider2.getNetwork()]);
  ...
  getAbi: async (address, provider2, storage) => {
    ...
    return (await import("./Multiwrap-EM67EBCY.js")).default;
  }
};
var NFTCollectionInitializer = {
  ...
  initialize: async function() {
    ...
    const [, provider2] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([NFTCollectionInitializer.getAbi(address, provider2, storage), import("./nft-collection-64b74d49.browser.esm-WVBQXMEO.js"), provider2.getNetwork()]);
  ...
  getAbi: async (address, provider2, storage) => {
    ...
    return (await import("./TokenERC721-C5NTLIZG.js")).default;
  }
};
var NFTDropInitializer = {
  ...
  initialize: async function() {
    ...
    const [, provider2] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([NFTDropInitializer.getAbi(address, provider2, storage), import("./nft-drop-1a6b2bae.browser.esm-GUWU4VQL.js"), provider2.getNetwork()]);
  ...
  getAbi: async (address, provider2, storage) => {
    ...
    const contractInfo = await getContractInfo(address, provider2);
    return !contractInfo || contractInfo.version > 3 ? (await import("./DropERC721-AEZLINSW.js")).default : (await import("./DropERC721_V3-YYP7WZX5.js")).default;
  }
};
var PackInitializer = {
  name: "Pack",
  ...
  initialize: async function() {
    ...
    const [, provider2] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([PackInitializer.getAbi(address, provider2, storage), import("./pack-890a3774.browser.esm-5EPAC7CU.js"), provider2.getNetwork()]);
  ...
  getAbi: async (address, provider2, storage) => {
    ...
    return AbiSchema.parse((await import("./Pack-FFEMZXFL.js")).default || []);
  }
};
var SignatureDropInitializer = {
  ...
  initialize: async function() {
    ...
    const [, provider2] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([SignatureDropInitializer.getAbi(address, provider2, storage), import("./signature-drop-63cb01b0.browser.esm-JVDI5MHW.js"), provider2.getNetwork()]);
  ...
  getAbi: async (address, provider2, storage) => {
    ...
    const contractInfo = await getContractInfo(address, provider2);
    return !contractInfo || contractInfo.version > 4 ? (await import("./SignatureDrop-ZA6OBPGT.js")).default : (await import("./SignatureDrop_V4-PIGVO2J4.js")).default;
  }
};
var SplitInitializer = {
  ...
  initialize: async function() {
    ...
    const [, provider2] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([SplitInitializer.getAbi(address, provider2, storage), import("./split-2ad42b19.browser.esm-J3WZM3Z6.js"), provider2.getNetwork()]);
  ...
  getAbi: async (address, provider2, storage) => {
    ...
    return (await import("./Split-B35A4IY5.js")).default;
  }
};
var TokenDropInitializer = {
  ...
  initialize: async function() {
    ...
    const [, provider2] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([TokenDropInitializer.getAbi(address, provider2, storage), import("./token-drop-4d5b8155.browser.esm-64PQMJFU.js"), provider2.getNetwork()]);
  ...
  getAbi: async (address, provider2, storage) => {
    ...
    const contractInfo = await getContractInfo(address, provider2);
    return !contractInfo || contractInfo.version > 2 ? (await import("./DropERC20-5NVXXCSJ.js")).default : (await import("./DropERC20_V2-LJUCHG6P.js")).default;
  }
};
var TokenInitializer = {
  ...
  initialize: async function() {
    ...
    const [, provider2] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([TokenInitializer.getAbi(address, provider2, storage), import("./token-1fce8b62.browser.esm-QAKLSKZN.js"), provider2.getNetwork()]);
  ...
  getAbi: async (address, provider2, storage) => {
    ...
    return (await import("./TokenERC20-XQEWXGEJ.js")).default;
  }
};
var VoteInitializer = {
  name: "VoteERC20",
  ...
  initialize: async function() {
    ...
    const [, provider2] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([VoteInitializer.getAbi(address, provider2, storage), import("./vote-76a03273.browser.esm-SAF7E5QK.js"), provider2.getNetwork()]);
  ...
  getAbi: async (address, provider2, storage) => {
    ...
    return (await import("./VoteERC20-AROSKRFG.js")).default;
  }
};
...
async function estimateTransactionCost(provider2, tx) {
  const chainId = (await provider2.getNetwork()).chainId;
  let l1GasCost = BigNumber.from(0);
  if (isOpStackChain(chainId)) {
    const {
      asL2Provider
    } = await import("./dist-SV3QXOK6.js");
    const l2RpcProvider = asL2Provider(provider2);
    l1GasCost = await l2RpcProvider.estimateL1GasCost(tx);
  }
  const gasLimit = tx.gasLimit || await provider2.estimateGas(tx);
  const gasPrice = await getGasPrice(provider2);
  const gasCost = BigNumber.from(gasLimit).mul(gasPrice);
  return gasCost.add(l1GasCost);
}
function isOpStackChain(chainId) {
  return chainId === c10.chainId || chainId === c11155420.chainId || chainId === c8453.chainId || chainId === c84532.chainId || chainId === c7777777.chainId || chainId === c999999999.chainId || chainId === c42220.chainId || chainId === c44787.chainId;
}
...
var Transaction = class extends TransactionContext {
...
async function enginePrepareRequest(transaction, signer, provider2, storage, gaslessOptions) {
  ...
  if (transaction.functionName === "approve" && transaction.functionArgs.length === 2) {
    ...
    const forwarderAddress = gaslessOptions.engine.relayerForwarderAddress || CONTRACT_ADDRESSES[transaction.chainId]?.openzeppelinForwarder || await computeForwarderAddress(provider2, storage);
    const ForwarderABI = (await import("./Forwarder-OZB5T7TG.js")).default;
    const forwarder = new Contract(forwarderAddress, ForwarderABI, provider2);
    const nonce = await getAndIncrementNonce(forwarder, "getNonce", [transaction.from]);
    let domain;
 ...
 * @license MIT
 */
//# sourceMappingURL=chunk-CFECTRPO.js.map
