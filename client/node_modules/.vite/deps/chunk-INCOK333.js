...
import {
  require_buffer_reverse,
  require_treeify
} from "./chunk-RLHLCECS.js";
import {
...
var Base = class {
  ...
  bufferIndexOf(array, element) {
    ...
    if (isSorted) {
      return this.binarySearch(array, element, import_buffer2.Buffer.compare);
    }
  ...
  static bufferify(value) {
    if (!import_buffer2.Buffer.isBuffer(value)) {
      if (typeof value === "object" && value.words) {
        return import_buffer2.Buffer.from(value.toString(convertWordsToBuffer), "hex");
      } else if (Base.isHexString(value)) {
        return import_buffer2.Buffer.from(value.replace(/^0x/, ""), "hex");
      } else if (typeof value === "string") {
        return import_buffer2.Buffer.from(value);
      } else if (typeof value === "bigint") {
        return import_buffer2.Buffer.from(value.toString(16), "hex");
      } else if (value instanceof Uint8Array) {
        return import_buffer2.Buffer.from(value.buffer);
      } else if (typeof value === "number") {
        let s = value.toString();
        if (s.length % 2) {
          s = `0${s}`;
        }
        return import_buffer2.Buffer.from(s, "hex");
      } else if (ArrayBuffer.isView(value)) {
        return import_buffer2.Buffer.from(value.buffer, value.byteOffset, value.byteLength);
      }
    }
    return value;
  }
  ...
  static bufferToHex(value) {
    let withPrefix = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : true;
    return `${withPrefix ? "0x" : ""}${(value || import_buffer2.Buffer.alloc(0)).toString("hex")}`;
  }
  bufferify(value) {
  ...
  bufferifyFn(f) {
    return (value) => {
      const v = f(value);
      if (import_buffer2.Buffer.isBuffer(v)) {
        return v;
      }
      if (this.isHexString(v)) {
        return import_buffer2.Buffer.from(v.replace("0x", ""), "hex");
      }
      if (typeof v === "string") {
        return import_buffer2.Buffer.from(v);
      }
      if (typeof v === "bigint") {
        return import_buffer2.Buffer.from(value.toString(16), "hex");
      }
      if (ArrayBuffer.isView(v)) {
        return import_buffer2.Buffer.from(v.buffer, v.byteOffset, v.byteLength);
      }
      ...
      return import_buffer2.Buffer.from(hexResult, "hex");
    };
  }
  isHexString(value) {
...
var MerkleTree = class extends Base$1 {
  constructor(leaves, hashFn) {
    let options = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : {};
    super();
    __publicField(this, "duplicateOdd", false);
    __publicField(this, "concatenator", import_buffer2.Buffer.concat);
    ...
    if (options.fillDefaultHash) {
      ...
      } else if (import_buffer2.Buffer.isBuffer(options.fillDefaultHash) || typeof options.fillDefaultHash === "string") {
  ...
  processLeaves(leaves) {
    ...
    if (this.sortLeaves) {
      this.leaves = this.leaves.sort(import_buffer2.Buffer.compare);
    }
  ...
  createHashes(nodes) {
    ...
    while (nodes.length > 1) {
      ...
      for (let i = 0; i < nodes.length; i += 2) {
        ...
        if (this.sortPairs) {
          combined.sort(import_buffer2.Buffer.compare);
        }
  ...
  getLeaves(values) {
    if (Array.isArray(values)) {
      if (this.hashLeaves) {
        ...
        if (this.sortLeaves) {
          values = values.sort(import_buffer2.Buffer.compare);
        }
      }
  ...
  getLayersFlat() {
    ...
    layers.unshift(import_buffer2.Buffer.from([0]));
    return layers;
  }
  getHexLayersFlat() {
  ...
  getRoot() {
    ...
    return this.layers[this.layers.length - 1][0] || import_buffer2.Buffer.from([]);
  }
  getHexRoot() {
  ...
  getProof(leaf, index) {
    ...
    if (!Number.isInteger(index)) {
      ...
      for (let i = 0; i < this.leaves.length; i++) {
        if (import_buffer2.Buffer.compare(leaf, this.leaves[i]) === 0) {
          index = i;
        }
      }
    }
  ...
  getMultiProof(tree, indices) {
    ...
    if (!indices.every(Number.isInteger)) {
      let els = indices;
      if (this.sortPairs) {
        els = els.sort(import_buffer2.Buffer.compare);
      }
  ...
  verify(proof, targetNode, root) {
    ...
    for (let i = 0; i < proof.length; i++) {
      ...
      } else if (import_buffer2.Buffer.isBuffer(node)) {
        data = node;
        isLeftNode = true;
      ...
      if (this.isBitcoinTree) {
        ...
        if (this.sortPairs) {
          if (import_buffer2.Buffer.compare(hash, data) === -1) {
    ...
    return import_buffer2.Buffer.compare(hash, root) === 0;
  }
  verifyMultiProof(root, proofIndices, proofLeaves, leavesCount, proof) {
    ...
    while (i < indexqueue.length) {
      ...
      if (index >= 2 && {}.hasOwnProperty.call(tree, index ^ 1)) {
        ...
        if (this.sortPairs) {
          pair = pair.sort(import_buffer2.Buffer.compare);
        }
  ...
  verifyMultiProofWithFlags(root, leaves, proofs, proofFlag) {
    ...
    for (let i = 0; i < totalHashes; i++) {
      ...
      const buffers = [bufA, bufB].sort(import_buffer2.Buffer.compare);
      hashes[i] = this.hashFn(this.concatenator(buffers));
    }
    return import_buffer2.Buffer.compare(hashes[totalHashes - 1], root) === 0;
  }
...
// node_modules/@thirdweb-dev/sdk/dist/signature-2e5c3f0d.browser.esm.js
init_shim();
init_lib();
...
//# sourceMappingURL=chunk-INCOK333.js.map
